...
chat_prompt = ChatPromptTemplate.from_messages(
    [
        ...
        ("human", "Please find professor information in the article below.\n{article}"),
    ]
)
...
def extract_professor_info(sid, article):
    try:
        print('sid:', sid)
        if pd.isna(article):
        ...
        new_str = re.sub(r"[^\uAC00-\uD7A30-9a-zA-Z~!@#$%&*()-=,.\s]", "", article)
        if sid == 19330:
            print(article)
            print(new_str)
...
df["result"] = df.apply(lambda x: extract_professor_info(x['sid'], x['article']), axis=1)
