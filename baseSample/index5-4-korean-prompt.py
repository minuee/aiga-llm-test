...
class Professor(BaseModel):
    """한 명의 교수에 대한 정보"""
    professor: str = Field(description="교수의 이름", required=True)
    hospital: str = Field(description="교수가 소속된 병원", required=True)
...
# Chain
llm = OllamaFunctions(model=LLAMA_MODEL, format="json", temperature=0)
# llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0)

structured_llm = llm.with_structured_output(Professors)
...
def extract_person_hospital_info(text_list):
    ...
    for text in text_list:
        try:
            result = chain.invoke({"context": text, "question":"한 문장에 포함되어 있는 교수를 최대 3명까지 찾아줘. 영어로 번역하지 말고, 그대로 한글로 알려줘"})
            print(result)
...
# Example usage
texts = [
    """저는 만71세 남자로 국내 최 장기 이식 생존자 처럼 ROTC 제대와 동시에 대기업에 입사하며 받은 신체검사에서
단백뇨가 검출되어 정밀 검사와 조직검사를 받고 앞으로 16년후에는 이식이나 투석을 받아야 됩니다 하는
경희의료원 김명재 교수의 진단 그대로 26세에에 진단후 41세에 바로 신장이식을 받아 현재 32년차로 Cr 1.46~1.5
BUN 28,사구체여과율 48 정도로 유지 하고 살고 있습니다

서울성모병원 양철우 교수님 진료를 받았으나 이제 곧 정년 하시므로 다른 교수님으로 주치의가 비뀌겠지요
그동안 아무 신장 관련 카페에 가입 하지 않고 지내다 신촌 세브란스병원에서 Cr이 1.69~1.77이 나온 일이 있었
기에 깜짝 놀라 관련 카페를 찾다가 여기 동호회가 신뢰 할 만하고 도움을 받을수 있는곳 임을 알고 가입하여 여러
환우님들과 더불어 건강 생활 하고 활동도 열심히 하고 싶습니다
환우님들 모두 지금보다 휠씬 나은 신약이나 의료 신기술이 개발되어 건강한 삶을 사시기 바랍니다 ~~~
혹시 누군가 저의 도움이 필요 하신분이 있으시면 글로서 도와 드리고 저도 좋은글을 읽고 삶의 희망을 가지고
살고 싶습니다
국내 최장기 이식신의 기록이 42년 이라 하여 저도 희망이 있답니다
고생하는 한우 여러분 모두 힘 내세요!"""
