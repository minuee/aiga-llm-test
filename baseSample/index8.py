...
def extract_person_hospital_info(sid, article):
    try:
        print('sid:', sid)
        result = chain.invoke({
          "article": article,
          "question": "글 내용(Article)에서 교수 정보를 최대 3명까지 찾아 주세요. 찾을 수 없으면 빈 값으로 형식만 맞춰주세요. 부연 설명은 필요 없습니다.",
        })
...
df["result"] = df.apply(lambda x: extract_person_hospital_info(x['sid'], x['article']), axis=1)
...
# df.to_csv("sns_recommand_result.csv", sep=',')