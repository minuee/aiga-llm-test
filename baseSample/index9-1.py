...
# sns_recommand.csv - 샘플 파일
# sns_recommand_100.csv - 샘플 파일( row 100 개)
df = pd.read_csv("./sns_recommand.csv", sep="|")

LLAMA_REMOTE = os.environ['LLAMA_REMOTE']
print("LLAMA_REMOTE:", LLAMA_REMOTE)

LLAMA_MODEL = os.environ['LLAMA_MODEL']
print("model:", LLAMA_MODEL)

class Professor(BaseModel):
    name: str = Field(description="교수의 이름")
...
#         (
#             "system",
#             # "You are an AI that extracts information from sentences.\n{format_instructions}",
#             "Answer user requests. Extract only the given format information.\n{format_instructions}",
#         ),
#         ("human", """In the article below, #content is the body of the post, and #comments are comments on the body of the post. Find professor information in this text and comments.\n#content:{content}\n#comments:{comments}"""),
#     ]
# )

sampleTemplate = """다음은 게시글과 댓글 데이터입니다. 이 게시글과 댓글에서 교수 정보를 추출하여 주세요.
{format_instructions}
...
chat_prompt = PromptTemplate(
    ...
    input_variables=["content", "comments"],
...
if LLAMA_REMOTE == "true":
    llm = ChatOllama(model=LLAMA_MODEL, temperature=0, base_url="http://ec2-3-233-233-233.compute-1.amazonaws.com:11434")
else:
...
samples = [
  {"sid": 0, "contents": contents1, "comments": comments1},
  {"sid": 1, "contents": contents2, "comments": comments2}
]
...
print(json_data)
with open('sns_recommand_result.json', 'w', encoding='utf-8') as f:
    f.write(json_data)